...
functions{
  ...
  vector triangle_model(vector t, int n, real T_P, real T_U, real V_P, real t_inf) {
    vector[n] vi; 
    real t_detectable = 4.0/V_P*T_P;
    real t_lag = t_detectable+t_inf;
    ...
    for(i in 1:n){
      ...
      else if(t[i] <t_lag+T_P){    
        vi[i]  = 2.0 + (V_P)/T_P*(t[i]-t_lag); 
      }
      else if(t[i] >=t_lag+T_P){
        vi[i] = V_P + 2.0 + (2.0-(V_P+2.0))/T_U*(t[i] - t_lag - T_P); 
      }
    }
    return vi;
  } 
...
// Approximation of the square function
  vector constant_model(vector t, int n, real T_P, real T_U, real V_P, real t_inf) {
    //vector[n] vi; 
    real t_detectable = 4.0/V_P*T_P;
    real t_lag = t_detectable+t_inf;
    real x0 = t_lag;
    real x1 = T_P+T_U+t_lag;
    real A = 5.0;
    vector[1] vi; 

    vi[1] = 0.0000001+1/(1+exp(-A*(t[1]-x0)))+exp(-A*(t[1]-x1))/(1+exp(-A*(t[1]-x1)))-1.0;

    return vi;
 } 

  real integral_constant_model(real t,  real T_P, real T_U, real V_P, real t_inf) {
    real VL_integral;
    VL_integral = 0;
    real t_detectable = 4.0/V_P*T_P;
    real t_lag = t_detectable+t_inf;
   ...
   real integral_constant_model_max_time(real T_P, real T_U ) {
      real VL_integral;
      VL_integral = T_P+T_U;
      return VL_integral;
   }  
...
parameters{
  ...
  //vector[n_infected_total]  TI_tmp; 
...
transformed parameters{
  ...
  real loglik;
 // matrix[n_households, max_infected] TS;
  ...
  for(k in 1:n_households){
    for(j in 1:n_infected[k]){     
      LVL[k,start[k,j]:end[k,j]] = to_row_vector(  triangle_model( to_vector(time[k,start[k,j]:end[k,j]]), n_time,  T_P[k,j], T_U[k,j], V_P[k,j], tinfection[k,j]) ) ; // VI is negligeble, so the observable log viral load is the non infectious viruses
  ...
  for(k in 1:n_infected_total){     
    if(symptomatic[k] == 1){
        LLsymptoms +=  normal_lpdf(log( TS[row_infected[k],col_infected[k]])|  log_Delta_S_mu, Delta_S_sd);  // estimate the distribution of the incubation period
     } 
  }
    ...
    for(k in 1:n_households){
      for(j in 2:n_infected[k]){  // we don't consider here the first individual who is the index case 
        ...
        for(i in 1:n_infected[k]){ // sum over the possible infectors of individual i
          if(i != j){
            Int= integral_constant_model( tinf[1],  T_P[k,i], T_U[k,i],V_P[k,i],  tinfection[k,i]); // the cumulative hazard for individual j up to time tinf[1] 
       ...
       P = 0;
        for(i in 1:n_infected[k]){ // sum over the possible infectors of individual i
          if(i != j){
            lvi = constant_model( tinf, 1, T_P[k,i], T_U[k,i],V_P[k,i], tinfection[k,i]); //   0 or 1
             P = P  +  lvi[1] ;
          }
        } 
      ...
      for(k in 1:n_households){
        ...
        for(j in 1:n_infected[k]){ // sum over the possible infectors of individual i
          //  Int3+= integral_constant_model( max_time, T_P[k,j], T_U[k,j], V_P[k,j], tinfection[k,j]);
            Int3+= integral_constant_model_max_time(T_P[k,j], T_U[k,j]);
          }  
